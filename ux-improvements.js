...
function updateStickyBar(verdictText, direction, speed, verdictColors) {
    const stickyVerdict = document.getElementById('sticky-verdict-badge');
    const stickyDirection = document.getElementById('sticky-direction');
    const stickySpeed = document.getElementById('sticky-speed');

    if (stickyVerdict) {
        stickyVerdict.textContent = verdictText || '--';
        // Apply verdict color to badge background
        stickyVerdict.style.background = '';
        stickyVerdict.style.color = '';
        if (verdictColors) {
            const colorMap = {
                'bg-green-300': { bg: '#86efac', color: '#14532d' },
                'bg-yellow-300': { bg: '#fde047', color: '#713f12' },
                'bg-orange-300': { bg: '#fdba74', color: '#7c2d12' },
                'bg-red-400': { bg: '#f87171', color: '#450a0a' },
                'bg-blue-200': { bg: '#bfdbfe', color: '#1e3a8a' },
                'bg-cyan-300': { bg: '#67e8f9', color: '#164e63' },
                'bg-purple-400': { bg: '#c084fc', color: '#3b0764' },
                'from-yellow-400': { bg: '#facc15', color: '#713f12' },
                'bg-amber-300': { bg: '#fcd34d', color: '#78350f' },
            };
            for (const cls of verdictColors) {
                if (colorMap[cls]) {
                    stickyVerdict.style.background = colorMap[cls].bg;
                    stickyVerdict.style.color = colorMap[cls].color;
                    break;
                }
            }
        }
    }
    if (stickyDirection) stickyDirection.textContent = direction || '--';
    if (stickySpeed) stickySpeed.textContent = speed !== null && speed !== undefined ? `${speed} kts` : '-- kts';
}

window.updateStickyBar = updateStickyBar;

function initStickyBar() {
    const stickyBar = document.getElementById('sticky-wind-bar');
    const windCard = document.getElementById('wind-highlight-card');
    if (!stickyBar || !windCard) return;

    window.addEventListener('scroll', () => {
        const cardBottom = windCard.getBoundingClientRect().bottom;
        if (cardBottom < 70) {
